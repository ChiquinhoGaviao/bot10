...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:nabuto\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=554184537851:+55 418453-7851\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[鉂梋 ESTE PEDIDO 脡 ESPEC脥FICO PARA USUARIO MOD Chiquinho BOT*',
			...
			const ownerNumber = ["554184537851@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"554184537851@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["554184537851@s.whatsapp.net"]//ubah nomor lo
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
			client.updatePresence(from, Presence.composing)
			reply("teu pai Aquele Veio Corno")
				}
	
	if (messagesC.includes("bosta")){
			client.updatePresence(from, Presence.composing)
			reply("quer chamar aten莽茫o? Manda A Foto Da Bct Da Sua Mae")
	}
		...
		if (messagesC.includes("vsfd")){
			client.updatePresence(from, Presence.composing)
			reply("bora juntos BB?")
	}
	
		if (messagesC.includes("cadebot")){
			client.updatePresence(from, Presence.composing)
			reply("olha eu aqui Pra Mim Se Nem Existe")
	}
	
		if (messagesC.includes("bot")){
			client.updatePresence(from, Presence.composing)
			reply("roi, Quer Me Da O BumBum?馃ズ鉂わ笍")
	}
...
//@nabutoYT					
    ...
    break
				...
				case 'premiumlist':
					...
					teks += `鈹�+ Total : ${premium.length}\n鈺扳攢鈹�鈹�鈹�鈹�鈹�*銆� *NABUTOBOT* 銆�*鈹�鈹�鈹�鈹�`
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`鈺攢鈹�鈹�鈹�*銆� *TOTAL DE USU脕RIOO NABUTO BOT 馃憫* 銆峔n\`\`\``
					no = 0
					for (let hehehe of user) {
						no += 1
						teks += `\`\`\`[${no.toString()}]\`\`\` @${hehehe.split('@')[0]}\n`
					}
					teks += `鈹�+ Total de usu谩rios : ${user.length}\n鈺扳攢鈹�鈹�鈹�鈹�鈹�*銆� *NABUTO* 銆�*鈹�鈹�鈹�鈹�`
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono 茅 esse gasoso a铆 tal de "nabuto"馃檮, chama ele a铆 ele diz que 茅 "Shitpost"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*鈺攢鈹�鈹�鈹�*銆� *ADMINBOT NABUTO 鉁�* 銆峔n*鈹�+ wa.me/554184537851*鈺扳攢鈹�鈹�鈹�鈹�鈹�*銆� *NABUTO* 銆�*鈹�鈹�鈹�鈹�*\n\n*_SE QUER SER ADMIN DO BOT NABUTO_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*鈺攢鈹�鈹�鈹�*銆� *PREMIUM USER馃憫* 銆峔n*鈹�+ wa.me/554184537851*鈺扳攢鈹�鈹�鈹�鈹�鈹�*銆� * NABUTO* 銆�*鈹�鈹�鈹�鈹�*\n\n*_SE QUER SER UM USU脕RIO PREMIUM DO NABUTO BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '锔�*canal do nabuto:*\n\n https://youtube.com/channel/UCZEtf9AlsC2zsJQwrfW-44w'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
